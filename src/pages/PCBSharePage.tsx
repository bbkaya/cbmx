...
import {
  ...
  updatePCBMemberRole,
  type PCBAccessRole,
  type PCBMemberListRow,
} from "../pcb/PCBData";

type ShareableRole = Exclude<PCBAccessRole, "owner">;
...
function memberLabel(row: PCBMemberListRow) {
  return row.display_name?.trim() || row.email || row.user_id;
}

export default function PCBSharePage() {
  const { pcbId } = useParams<{ pcbId: string }>();
  const { user } = useAuth();
  const navigate = useNavigate();

  const [pcbName, setPcbName] = useState("");
  const [role, setRole] = useState<PCBAccessRole | null>(null);
  const [members, setMembers] = useState<PCBMemberListRow[]>([]);
  ...
  async function refresh() {
    if (!user?.id || !pcbId) return;

    setIsLoading(true);
    setError(null);

    try {
      const [pcb, myRole, memberRows] = await Promise.all([
        loadPCB(pcbId),
        getPCBRole(pcbId, user.id),
        listPCBMembers(pcbId),
      ]);
      ...
      if (!myRole) {
        setMembers([]);
        setError("You do not have access to this Process Canvas.");
        return;
      }
  ...
  async function handleRemove(userId: string) {
    ...
    const ok = window.confirm("Remove this member from the Process Canvas?");
    if (!ok) return;

    setIsBusy(true);
    try {
      await removePCBMember({
        pcbId,
        userId,
      });
      await refresh();
  ...
  async function handleLeave() {
    if (!pcbId) return;
    if (role === "owner") {
      window.alert("The owner cannot leave their own Process Canvas.");
      return;
    }

    const ok = window.confirm("Leave this shared Process Canvas?");
    if (!ok) return;

    setIsBusy(true);
    try {
      await leavePCB(pcbId);
      navigate("/app/pcbs");
    } catch (err: any) {
      window.alert("Could not leave Process Canvas: " + String(err?.message ?? err));
    } finally {
      setIsBusy(false);
    }
  }
    ...
    return (
      <div style={{ maxWidth: 960 }}>
        <div style={{ marginBottom: 12 }}>
          <Link to="/app/pcbs">← Back to My Process Canvases</Link>
        </div>
        <div style={card}>
          <div style={{ fontWeight: 800, marginBottom: 8 }}>Could not open sharing page</div>
          <div style={{ color: "#475569" }}>{error || "Missing Process Canvas id."}</div>
        </div>
      </div>
    );
  }
    ...
    <div style={{ maxWidth: 1040 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        <div>
          ...
          <h2 style={{ margin: 0 }}>Share Process Canvas</h2>
          <div style={{ color: "#64748b", marginTop: 4 }}>{pcbName}</div>
        </div>

        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <Link to={`/app/pcb/${pcbId}`}>
            <button type="button" style={buttonStyle}>Open Process Canvas</button>
          </Link>
